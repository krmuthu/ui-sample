# AutoSuggest Component

## Overview

Unique identifier for the option

## Installation

```bash
npm install clipper-ui
# or
yarn add clipper-ui
```

## Usage

```jsx
import { AutoSuggest } from 'clipper-ui';

function MyComponent() {
  return (
    <AutoSuggest 
      variant="primary" 
      size="md"
    >
      Content goes here
    </AutoSuggest>
  );
}
```

## Props

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `variant` | `'default' \| 'primary' \| 'secondary'` | `'default'` | Visual variant of the component |
| `size` | `'sm' \| 'md' \| 'lg'` | `'md'` | Size of the component |
| `disabled` | `boolean` | `false` | Whether the component is disabled |
| `className` | `string` | `''` | Additional CSS class names |
| `onClick` | `(event: React.MouseEvent<HTMLElement>) => void` | - | Callback fired when the component is clicked |
| `children` | `React.ReactNode` | - | Content of the component |

## Examples

### Basic Usage

```jsx
<AutoSuggest>Default AutoSuggest</AutoSuggest>
```

### Variants

```jsx
<AutoSuggest variant="primary">Primary AutoSuggest</AutoSuggest>
<AutoSuggest variant="secondary">Secondary AutoSuggest</AutoSuggest>
```

### Sizes

```jsx
<AutoSuggest size="sm">Small AutoSuggest</AutoSuggest>
<AutoSuggest size="md">Medium AutoSuggest</AutoSuggest>
<AutoSuggest size="lg">Large AutoSuggest</AutoSuggest>
```

### Disabled State

```jsx
<AutoSuggest disabled>Disabled AutoSuggest</AutoSuggest>
```

## Accessibility

- The component sets appropriate `aria-disabled` attributes based on the `disabled` prop.
- Use the `aria-label` prop to provide accessible names when needed.

## Best Practices

- Use primary variant for main actions
- Use default variant for general content
- Ensure there is sufficient color contrast for all variants

## Design Considerations

- The component supports both light and dark themes
- Styling is based on a consistent design system
- Typography is handled automatically based on size 
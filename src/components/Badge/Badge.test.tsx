import React from 'react';
import { render, screen, fireEvent } from '@testing-library/react';
import { Badge } from './Badge';

describe('Badge Component', () => {
  // Basic rendering test
  it('renders correctly with default props', () => {
    render(<Badge>Test Content</Badge>);
    
    const component = screen.getByText('Test Content');
    expect(component).toBeInTheDocument();
  });

  // Props testing
  it('applies variant classes correctly', () => {
    const { rerender } = render(
      <Badge variant="primary" data-testid="test-component">
        Primary Variant
      </Badge>
    );
    
    let component = screen.getByTestId('test-component');
    expect(component).toHaveClass('bg-blue-100');
    
    rerender(
      <Badge variant="secondary" data-testid="test-component">
        Secondary Variant
      </Badge>
    );
    
    component = screen.getByTestId('test-component');
    expect(component).toHaveClass('bg-gray-100');
  });

  it('applies size classes correctly', () => {
    const { rerender } = render(
      <Badge size="sm" data-testid="test-component">
        Small Size
      </Badge>
    );
    
    let component = screen.getByTestId('test-component');
    expect(component).toHaveClass('p-2');
    
    rerender(
      <Badge size="lg" data-testid="test-component">
        Large Size
      </Badge>
    );
    
    component = screen.getByTestId('test-component');
    expect(component).toHaveClass('p-4');
  });

  // Behavior testing
  it('handles click events', () => {
    const handleClick = jest.fn();
    render(
      <Badge onClick={handleClick} data-testid="test-component">
        Clickable
      </Badge>
    );
    
    const component = screen.getByTestId('test-component');
    fireEvent.click(component);
    
    expect(handleClick).toHaveBeenCalledTimes(1);
  });

  it('does not trigger click events when disabled', () => {
    const handleClick = jest.fn();
    render(
      <Badge onClick={handleClick} disabled data-testid="test-component">
        Disabled
      </Badge>
    );
    
    const component = screen.getByTestId('test-component');
    fireEvent.click(component);
    
    expect(handleClick).not.toHaveBeenCalled();
    expect(component).toHaveAttribute('aria-disabled', 'true');
  });

  // Accessibility testing
  it('applies aria attributes correctly', () => {
    render(
      <Badge aria-label="Test label" data-testid="test-component">
        Accessible
      </Badge>
    );
    
    const component = screen.getByTestId('test-component');
    expect(component).toHaveAttribute('aria-label', 'Test label');
  });
}); 
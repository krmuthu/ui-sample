...
function main() {
  // Read input tokens
  const inputPath = path.join(process.cwd(), 'src/styles/design-tokens.json');
  const outputPath = path.join(process.cwd(), 'src/styles/transformed-tokens.json');

  try {
    // Read and parse the tokens file
    const tokensData = fs.readFileSync(inputPath, 'utf8');
